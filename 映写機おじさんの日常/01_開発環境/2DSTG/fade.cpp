		...
		{ //2フレーム経ったら
			...
			{ //テクスチャカウントがFADE_MAX以上のとき
				if (CManager::GetMode() == CManager::MODE_GAME ||
					...
					CManager::GetMode() == CManager::MODE_START_OVER)
				{
					...
					else if (CManager::GetMode() == CManager::MODE_SCORE_ATTACK ||
							 CManager::GetMode() == CManager::MODE_TUTORIAL_SCORE_ATTACK ||
							 CManager::GetMode() == CManager::MODE_SCORE_SCREEN ||
							 m_mode == CManager::MODE_START_OVER)
					{
						...
						else if (m_mode == CManager::MODE_SELECT ||
						...
						else if (m_mode == CManager::MODE_START_OVER)
						{
							if (CManager::GetModePointer() != NULL)
							{
								CGame *pGame = dynamic_cast<CGame*>(CManager::GetModePointer());
								CScoreAttack *pScoreAttack = dynamic_cast<CScoreAttack*>(CManager::GetModePointer());
								...
								{ //スコアアタックモードのとき
									...
									m_mode = CManager::MODE_SCORE_ATTACK;
								}
							}
						}

						if (CManager::GetMode() != CManager::MODE_SCORE_ATTACK && m_mode != CManager::MODE_SCORE_ATTACK)
						{
							if (CManager::GetPlayer()->GetPlayerLife() < PLAYER_LIFE)
							{ //ライフが減っていたとき
							  //ライフの初期化
